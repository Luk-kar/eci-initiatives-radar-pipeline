...
class TestFindLatestCsv:
    def test_find_latest_csv_returns_lexicographically_latest_match(
        self, tmp_path: Path
    ):
...
class TestValidateCsvExists:
    def test_validate_csv_exists_accepts_csv_with_header_and_data_row(
        self, tmp_path: Path
    ):
...
class TestLoadCsv:
    def test_load_csv_returns_rows_from_csv(self, tmp_path: Path):
        ...
        path.write_text(
            "registration_number,commission_answer\n" "2012/000001,\"['Answer 1']\"\n",
            encoding="utf-8",
        )
