...
- `Law_Passed`: A list of strings containing the exact text spans matched by the spaCy `LAW_MENTIONED` rules (evaluating lemmas like "apply", "adopt", or "force" in proximity to legislative terms). This must be computed by combining the text from the `commission_answer` and `followup_events` columns.
- `Is_Law_Passed`: A boolean (True/False) that is True if the `Law_Passed` list is not empty.
- `Rejected_Legislation`: A boolean (True/False) that is True if the combined text of `commission_answer` and `followup_events` triggers any of the spaCy `REJECTED_LEGISLATION` patterns (e.g., direct rejection, refusal to repeal, withdrawal, or outside competence).
