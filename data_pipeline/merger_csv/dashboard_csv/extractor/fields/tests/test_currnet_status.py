...
# ---------------------------------------------------------------------------
# Direct mapping for every non-answered source value
# ---------------------------------------------------------------------------
...
def test_direct_mapping(raw: str, expected: str) -> None:
    """Every non-answered source value maps to its dashboard label."""

    assert extract(raw, None, None) == expected


def test_status_map_covers_all_source_values() -> None:
...
def test_status_map_targets_only_canonical_dashboard_labels() -> None:
...
# ---------------------------------------------------------------------------
# Normalisation: whitespace / trailing asterisks / surrounding padding
# ---------------------------------------------------------------------------
...
def test_verification_normalisation(raw: str) -> None:
...
# ---------------------------------------------------------------------------
# Empty / None / unknown raw_status -> ValueError
# ---------------------------------------------------------------------------


@pytest.mark.parametrize("raw", ["", "   ", "\n\t", "*", "  *  "])
def test_empty_raw_status_raises(raw: str) -> None:
...
def test_none_raw_status_raises() -> None:
...
def test_unknown_status_error_carries_original_and_normalised() -> None:
...
def test_case_sensitivity_is_strict() -> None:
...
# ---------------------------------------------------------------------------
# "Answered initiative" upgrade matrix
# ---------------------------------------------------------------------------
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # ``Law Passed`` wins when both flags are truthy — it is the more
        # specific outcome.
        (True, False, "Law Passed"),
    ],
)
def test_answered_initiative_upgrade(
    is_law_passed: bool, rejected_legislation: bool, expected: str
) -> None:
    """The legislation flags refine ``Answered initiative`` into a verdict."""
    assert (
        extract("Answered initiative", is_law_passed, rejected_legislation) == expected
    )


def test_answered_initiative_both_flags_true_raises() -> None:
...
def test_answered_initiative_upgrade_after_normalisation() -> None:
...
# ---------------------------------------------------------------------------
# Mandated-bool contract for "Answered initiative"
# ---------------------------------------------------------------------------
...
def test_answered_initiative_missing_flags_raises(
    is_law_passed: bool | None, rejected_legislation: bool | None
) -> None:
...
def test_missing_flags_error_carries_both_values() -> None:
...
# ---------------------------------------------------------------------------
# Defence-in-depth: legislation flags must not affect non-answered statuses
# ---------------------------------------------------------------------------


@pytest.mark.parametrize("raw, expected", _NON_ANSWERED_CASES)
def test_non_answered_ignores_legislation_flags(raw: str, expected: str) -> None:
