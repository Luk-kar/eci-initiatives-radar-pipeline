...
filenames, filters metadata from responses_list.csv, and extracts an
...
class ECIResponseExtractor:
    """Top-level orchestrator for the ECI Initiative Details extraction pipeline."""
    ...
    def __init__(
        ...
        self.output_csv_name = ECI_RESPONSES_CSV_PATTERN.format(timestamp=timestamp)
        self.logger: Optional[logging.Logger] = None

    def run(self):
        """Main execution method."""

        session_path = self._find_latest_scrape_session()

        if not session_path:
            raise FileNotFoundError(f"No scraping session found in: {self.data_root}")

        log_dir = session_path / LOG_DIR_NAME
        self.logger = setup_logger(log_dir_path=log_dir, timestamp=self.timestamp)

        self.logger.info("Starting ECI initiative details extraction")
        self.logger.info(f"Session: {session_path.name}")

        html_dir = session_path / RESPONSES_DIR_NAME
        output_csv = session_path / self.output_csv_name

        initiatives_csv = self._find_latest_initiatives_csv(session_path)

        self.logger.info(html_dir)

        if not html_dir.exists():
            raise FileNotFoundError(f"HTML directory not found: {html_dir}")

        self.logger.info(initiatives_csv)

        if not initiatives_csv.exists():
        ...
        html_files_by_reg = self._collect_html_files(html_dir)
        ...
        self.logger.info(f"Found {number_of_responses} HTML response files")

        # Step 2: filter CSV metadata to only the reg numbers found on disk
        metadata_by_reg = self._load_responses_metadata(
            initiatives_csv, reg_numbers=set(html_files_by_reg.keys())
        )
        self.logger.info(f"Matched {len(metadata_by_reg)} CSV records to HTML files")
        ...
        for reg_number, html_path in html_files_by_reg.items():
            ...
            record = self._process_single(reg_number, csv_record, html_path)
        ...
        self.logger.info(f"Done. {len(records)} records written to {output_csv}")

    def _find_latest_initiatives_csv(self, session_path: Path) -> Optional[Path]:
        """
    ...
    def _process_single(
        self, reg_number: str, csv_record: dict, html_path: Path
        ...
        try:
            ...
            self.logger.error(f"Failed to process {reg_number}: {e}", exc_info=True)
            return None

    def _collect_html_files(self, html_dir: Path) -> Dict[str, Path]:
        """
        ...
        for html_file in html_dir.glob("*/*.html"):
            ...
            if not match:
                self.logger.debug(f"Skipping unrecognised filename: {html_file.name}")
                continue
    ...
    def _find_latest_scrape_session(self) -> Optional[Path]:
    ...
    def _load_responses_metadata(
        self, csv_path: Path, reg_numbers: set
    ) -> Dict[str, dict]:
        """
    ...
    def _write_csv(self, records: List[ECIResponseRecord], output_csv: Path):
        ...
        self.logger.info(f"Wrote {len(records)} rows to {output_csv}")
